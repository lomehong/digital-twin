...
- **结清**：主人消息入账（方向一）且模型消化对应请求 → 显式结算；关联目标
  `[目标·完成]` 销账时自动结清；**主人经数字分身·今日待办逐条答复（v0.10 增）**：
  `POST /dsh-mind/asks/answer` 结清该单 + 答复沿 `/say` 同路径注入 message_in 并
  反应性唤醒（请求单主 surfaces：右下角存在体「等你给」角标 → 数字分身·今日待办
  「心智请求单」，导航契约=顶部 Tab 标签 + `dsh-twin:focus-todo` 焦点事件，双方
  各持常量拷贝——宪章 §3.1 零运行时共享）
